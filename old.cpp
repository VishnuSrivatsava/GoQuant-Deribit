...
// Global log file
std::ofstream logFile("market_data_original.log", std::ios::app);
std::ofstream latencyLogFile("latency_data_original.log", std::ios::app); // Log file for latency data
std::mutex outputMutex;
std::queue<std::string> messageQueue;
bool running = true;

// Subscriptions map
std::map<std::string, std::set<websocket::stream<tcp::socket>*>> subscriptions;
...
// Function to get order book details
void getOrderBook(const std::string& accessToken, const std::string& instrument) {
    ...
    // Display order book details if available
    if (responseJson.contains("result")) {
        ...
        std::cerr << "Error: Could not retrieve order book data." << std::endl;
    }
}
...
int main() {
    ...
    if (!accessToken.empty()) {
        ...
        std::thread wsServerThread(runWebSocketServer);
        wsServerThread.detach();

        // Start orderbook updates in a separate thread
        std::thread orderbookThread(streamOrderbookUpdates);
        orderbookThread.detach();

        // Start message display in a separate thread
        std::thread displayThread(displayMessages);
